   ...
   public class AuthUtil
    {
        ...
        /// <returns>System.String.</returns>
        public static LoginResultResponse Login(string appKey, string username, string pwd)
        {
            var requestUri = "/api/check/login";
            //var a = SSOAuthUtil.Parse(new SSO.Login.PassportLoginRequest { Account = username, AppKey = appKey, Password = pwd });
            try
            {
                ...
                var result = JsonHelper.Instance.Deserialize<LoginResultResponse>(value);
                return result;
